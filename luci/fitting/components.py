"""
Reconstruct the individual line components of a fit.

Moved from ``LUCI.LuciConvenience``; the region-masking half of that module now
lives in ``LUCI.engine.selection``.
"""
...
def get_individual_components(fit_dictionary):
    ...
    fit_solution = fit_dictionary[
        "fit_sol"
    ]  # Solutions of the following form [line_1_amp, line_1_vel, line_1_broad, line_2_amp, ..., continuum]
    ...
    for ct, fit_line in enumerate(fit_lines):
        if (
            fit_line in fit_vectors.keys()
        ):  # Line has already been in fit_vectors so we need to append a number to its name -- # TODO: Only works for 2 components -- must generalize
            name = fit_line + "_2"
        else:
            name = fit_line
