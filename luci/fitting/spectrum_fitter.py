...
class SpectrumFitter:
    ...
    def sigma_constraints(self):
        ...
        return sigma_group_constraints(self.sigma_rel) + sigma_bounds(self.sigma_rel)

    def vel_constraints(self):
        """Tie the velocities of lines sharing a group in vel_rel."""
        return velocity_constraints(self.vel_rel, self.lines, self.line_dict, self.axis_step)

    def NII_constraints(self):
        """Hold NII6548 at one third the flux of NII6583."""
        return nii_doublet_constraint(self.lines, self.model_type, self.sinc_width)

    def multiple_component_vel_constraint(self):
        """Require each component to sit at a different position."""
        return distinct_position_constraints(self.lines)

    def amplitude_constraint(self):
        """Keep amplitudes and continuum inside the normalised range."""
        return amplitude_constraints(self.lines)
    ...
    def fit(self, sky_line=False):
        ...
        if sky_line != True:
            ...
            sigmas_errors = []
            flux_errors = []
            # if self.initial_conditions is False or self.initial_conditions is None:
...
# ``Fit`` was this class's name for its whole published life; keep it working.
Fit = SpectrumFitter
