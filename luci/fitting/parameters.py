"""
Conversions from raw fit-solution parameters to physical quantities:
velocity, velocity dispersion (broadening), and flux, plus their uncertainties.

Moved verbatim from ``LUCI.LuciFitParameters`` (which now re-exports these) as
part of grouping the fitting code under ``LUCI.fitting``.  Numerics unchanged.

Note: ``calculate_flux`` for the ``gaussian`` model carries an extra
``1.20671/FWHM_COEFF`` factor (~0.512x the textbook Gaussian integral).  That is
LUCI's long-standing convention, pinned by
``tests/test_pure_functions.py::test_gaussian_flux_uses_lucis_sinc_scaled_normalisation``
and flagged as decision C1 in REFACTOR_BUGS.md -- it is preserved here, not
"corrected".
"""
...
def calculate_broad_err(ind, fit_sol, axis_step, uncertainties):
    ...
    try:
        if fit_sol[3 * ind + 1] > 0 and fit_sol[3 * ind + 2] > 0:
            broad = (SPEED_OF_LIGHT * fit_sol[3 * ind + 2]) / fit_sol[3 * ind + 1]
            uncertainty_prop = np.sqrt(
                (uncertainties[3 * ind + 2] / fit_sol[3 * ind + 2]) ** 2
...
def calculate_flux_err(ind, fit_sol, uncertainties, model_type, sinc_width):
    ...
    if model_type == "gaussian":
        ...
        flux_err = calculate_flux(p0, p2, model_type, sinc_width) * np.sqrt(
            (p0_err / p0) ** 2
            + (p2_err / p2) ** 2
            * (sps.erf(p2 / c_0) - (2 * np.pi) / (np.sqrt(np.pi) * c_0) * np.exp(-(p2**2) / c_0**2)) ** 2
        )
