...
class Spectrum:
    ...
    def steps_from_resolution(self):
        ...
        self.n_steps = int(
            np.ceil(1.20671 * self.resolution / (2 * temp * self.delta_x))
        )  # The constant comes from the sinc function)
    ...
    def gaussian_model(self, channel, amp, pos, sigma):
        ...
        """
        f1 = Gaussian().evaluate(
            channel, [amp, pos, sigma], 1
        )  # The last argument is 1 since we add one line at a time
        return f1
