...
def add_lines(header, integrated_spectrum):
    ...
    if filter == "SN2":
        plt.vlines(
            486.1, 0.8 * min_y, 1.2 * max_y, linestyles="dashed", colors="coral", linewidths=(2,), label=r"H$\beta$"
        )
        plt.vlines(
            495.9, 0.8 * min_y, 1.2 * max_y, linestyles="dashed", colors="seagreen", linewidths=(2,), label="OIII4959"
        )
        plt.vlines(
            500.7,
            0.8 * min_y,
    ...
    if filter == "SN3":
        plt.vlines(
            656.3, 0.8 * min_y, 1.2 * max_y, linestyles="dashed", colors="coral", linewidths=(2,), label=r"H$\alpha$"
        )
        plt.vlines(
            658.3, 0.8 * min_y, 1.2 * max_y, linestyles="dashed", colors="seagreen", linewidths=(2,), label="NII6583"
        )
        plt.vlines(
            654.8, 0.8 * min_y, 1.2 * max_y, linestyles="dashed", colors="forestgreen", linewidths=(2,), label="NII6548"
        )
        plt.vlines(
            673.1, 0.8 * min_y, 1.2 * max_y, linestyles="dashed", colors="lightpink", linewidths=(2,), label="NII6583"
        )
        plt.vlines(
            671.6, 0.8 * min_y, 1.2 * max_y, linestyles="dashed", colors="plum", linewidths=(2,), label="NII6548"
        )
    if filter == "SN4":
        plt.vlines(
            656.3, 0.8 * min_y, 1.2 * max_y, linestyles="dashed", colors="coral", linewidths=(2,), label=r"H$\alpha$"
...
def visualize(deep_image, spectrum_axis, cube_final, header):
    ...
    plt.imshow(
        scaled_deep_image,
        origin="lower",
        vmin=np.percentile(scaled_deep_image, 5),
        vmax=np.percentile(scaled_deep_image, 99),
    )
