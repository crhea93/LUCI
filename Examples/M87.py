...
spectrum_axis = np.linspace(start, end, len_wl)*(redshift+1)# + np.linspace(start, end, len_wl)*redshift#np.arange(start, end, step)# + np.arange(start, end, step)*redshift
#spectrum_axis, sky_spectrum = cube.extract_spectrum(1100, 1100, 3)

# Machine Learning Reference Spectrum
ref_spec = fits.open('Reference-Spectrum-R1000.fits')[1].data
channel = []
counts = []
...
x_min = 1000
x_max = 1100#cube.shape[0]
y_min = 1000
y_max = 1100#cube.shape[1]
# Read in data
...
model_ML = keras.models.load_model('R1000-PREDICTOR-I')  # Read in
start = time.time()
print('Fitting...')
...
print(end-start)
# Save
fits.writeto('M87_VEL.fits', VEL.T, header, overwrite=True)
fits.writeto('M87_BROAD.fits', BROAD.T, header, overwrite=True)
