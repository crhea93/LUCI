...
class Luci():
    ...
    def read_in_cube(self):
        ...
        translate the header to standard wcs format.
        """
        print('Reading in data...')
        file =  h5py.File(self.cube_path+'.hdf5', 'r')
        self.quad_nb = file.attrs['quad_nb']#div_nb**2
        self.dimx = file.attrs['dimx']
        self.dimy = file.attrs['dimy']
        self.dimz = file.attrs['dimz']
        ...
        for iquad in tqdm(range(self.quad_nb)):
            ...
            self.cube_final[xmin:xmax, ymin:ymax, :] = iquad_data
    ...
    def spectrum_axis_func(self):
        ...
        self.spectrum_axis = np.linspace(start, end, len_wl)*(redshift+1)
    ...
    def fit_cube(self, lines, fit_function, x_min, x_max, y_min, y_max):
        ...
        for i in tqdm(range(x_max-x_min)):
            ...
            for j in range(y_max-y_min):
                # Calculate velocity
                y_pix = y_min+j
                sky = self.cube_final[x_pix, y_pix, :]# np.real(cube_final[i,j,:])
        ...
        return velocity_fits, broadening_fits
